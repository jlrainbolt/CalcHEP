...
// 2^(-50) = 9E-16 
 ...
 int plost= (eps< 1E-14*(fabs(a)+fabs(b))/fabs(b-a));
  ...
  if(i || plost || depth>50 || (*nErr)> nErrMax)
  { *ans+=s3*(b-a);
    *aAns+=(fabs(f[0])+4*fabs(f[2])+2*fabs(f[4])+4*fabs(f[6])+fabs(f[8]))
          *fabs(b-a)/12;
    if((*nErr)<=nErrMax &&( plost || depth>50))
    {
       simpson_err++;
       (*nErr)++;
  ...
  r_simpson(func,f,a,(a+b)/2,eps,aEps,ans,aAns,depth+1,nErr);
  for(i=0;i<5;i++) f[2*i]=f1[i];
  for(i=1;i<8;i+=2) f[i]=(*func)((a+b)/2+i*(b-a)/16);
  r_simpson(func, f,(a+b)/2,b,eps,aEps,ans,aAns,depth+1,nErr);
}
...
//  printf("i=%d esp=%E fabs(s3)=%E  fabs(s3-s2)=%E e_err=%E fabs(s3-s1)=%E 16*e_err=%E\n",i,eps, fabs(s3), fabs(s3-s2), e_err,fabs(s3-s1), 16*e_err);
// 2^(-50) = 9E-16
  int plost= (eps< 1E-14*(fabs(a)+fabs(b))/fabs(b-a));
  ...
  r_simpson_arg(func,f,a,(a+b)/2,par,eps,aEps,ans,aAns,depth+1,nErr);
  for(i=0;i<5;i++) f[2*i]=f1[i];
  for(i=1;i<8;i+=2) f[i]=(*func)((a+b)/2+i*(b-a)/16,par);
  r_simpson_arg(func, f,(a+b)/2,b,par,eps,aEps,ans,aAns,depth+1,nErr);
}
...
double simpson_arg( double (*func)(double,void*),void*par,double a,double b, double  eps)
{
  ...
  for(j=0;;j++)
  {  double ans=0., aAns=0.; 
     r_simpson_arg(func,f,a,b,par,eps,&aEps,&ans,&aAns,0,&nErr);
     if(5*aAns*eps > aEps || j>=2 ) return ans;
     if(!isfinite(aAns)) return aAns;
